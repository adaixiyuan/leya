...
#define MAX_SHOW_FAVOURITE_NUM 3
#define MAX_SHOW_COMMON_FRIEND_NUM 3
...
- (NSString *)getCategoryString {
    ...
    if (i > 0) {
        if ([self.maryCategory count] > MAX_SHOW_FAVOURITE_NUM) {
            strFavourite = [NSString stringWithFormat:@"等%ld个爱好", (unsigned long)[self.maryCategory count]];
            [strFavouriteTotal appendString:strFavourite];
        }
    }
    else {
