...
- (void)commonInit {
    if (![_database tableExists:@"messages"]) {
        [_database executeUpdate:@"create table \"messages\" (\"username\" text, \"fromid\" text, \"toid\" text, \"type\" text, \"message\" text, \"object\" text, \"time\" integer, \"width\" double, \"height\" double, \"isread\" integer default 0)"];
    }
...
- (void)sendMessage:(NSString *)message toPeerId:(NSString *)peerId {
    ...
    [dict setObject:[UserData currentUser].username forKey:@"username"];
    
    [_database executeUpdate:@"insert into \"messages\" (\"username\", \"fromid\", \"toid\", \"type\", \"message\", \"time\", \"isread\") values (:username, :fromid, :toid, :type, :message, :time, 1)" withParameterDictionary:dict];
    [[NSNotificationCenter defaultCenter] postNotificationName:NOTIFICATION_MESSAGE_UPDATED object:nil userInfo:dict];
}

- (void)sendAttachment:(AVObject *)object toPeerId:(NSString *)peerId width:(double)dWidth height:(double)dHeight {
...
//    AVFile *file = [object objectForKey:type];
    ...
    [_database executeUpdate:@"insert into \"messages\" (\"username\", \"fromid\", \"toid\", \"type\", \"object\", \"time\", \"width\", \"height\", \"isread\") values (:username, :fromid, :toid, :type, :object, :time, :width, :height, 1)"
...
- (NSArray *)getMessagesForPeerId:(NSString *)peerId {
    ...
    NSString *strUsername = [UserData currentUser].username;
    FMResultSet *rs = [_database executeQuery:@"select \"fromid\", \"toid\", \"type\", \"message\", \"object\", \"time\", \"width\", \"height\", \"isread\" from \"messages\" where \"username\"=? and ((\"fromid\"=? and \"toid\"=?) or (\"fromid\"=? and \"toid\"=?))"
...
- (void)session:(AVSession *)session didReceiveMessage:(AVMessage *)message {
    ...
    if ([type isEqualToString:@"text"]) {
        ...
        [_database executeUpdate:@"insert into \"messages\" (\"username\", \"fromid\", \"toid\", \"type\", \"message\", \"time\") values (:toid, :fromid, :toid, :type, :message, :time)" withParameterDictionary:dict];
    } else {
        ...
        [_database executeUpdate:@"insert into \"messages\" (\"username\", \"fromid\", \"toid\", \"type\", \"object\", \"time\", \"width\", \"height\") values (:toid, :fromid, :toid, :type, :object, :time, :width, :height)" withParameterDictionary:dict];
    }
