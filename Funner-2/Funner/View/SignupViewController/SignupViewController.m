...
- (IBAction)onButSignup:(id)sender {
    ...
    if ([self.mTxtPhoneNumber.text length] == 0) {
        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"" message:@"请输入您的手机号码" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
    ...
    if ([self.mTxtPassword.text length] == 0) {
        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"" message:@"请输入您的密码" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
        return;
    }
    
    if ([self.mTxtVerifyCode.text length] == 0) {
        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"" message:@"请输入验证吗" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
        return;
    }
...
- (IBAction)onButVerifyCode:(id)sender {
    ...
    if ([self.mTxtPhoneNumber.text length] == 0) {
        UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"" message:@"请输入您的手机号" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
        [alert show];
