...
//
    ...
    [[CDSessionManager sharedInstance] setUnreadToReadForPeerId:self.mUser.username];
...
- (void)messageUpdated:(NSNotification *)notification {
    ...
    maryMessages = [[CDSessionManager sharedInstance] getMessagesForPeerId:self.mUser.username];
    [[CDSessionManager sharedInstance] setUnreadToReadForPeerId:self.mUser.username];
...
- (IBAction)onButSend:(id)sender {
    ...
    [[CDSessionManager sharedInstance] sendMessage:self.mTxtContent.text toPeerId:self.mUser.username];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [self configureCell:tableView cellForRowAtIndexPath:indexPath modifyCell:YES];
...
- (UITableViewCell *)configureCell:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath modifyCell:(BOOL)bModify {
    ...
    NSDictionary *dictMsg = [maryMessages objectAtIndex:indexPath.row];
    NSString *strFromId = dictMsg[@"fromid"];
    NSString *strType = dictMsg[@"type"];
    
    BOOL bNeedShowTime = YES;
    if (indexPath.row > 0) {
        NSDictionary *dictPrevMsg = [maryMessages objectAtIndex:indexPath.row - 1];
    ...
    if ([strFromId isEqualToString:self.mUser.username]) {
        if ([strType isEqualToString:@"text"]) {
            ...
            if (bModify) {
                ...
                if ([msgImageInCell.mImgViewPhoto.gestureRecognizers count] == 0) {
                    ...
                    msgImageInCell.mImgViewPhoto.tag = indexPath.row;
                }
            }
        ...
        if ([strType isEqualToString:@"text"]) {
            ...
            if ([msgImageOutCell.mImgViewPhoto.gestureRecognizers count] == 0) {
                ...
                msgImageOutCell.mImgViewPhoto.tag = indexPath.row;
            }
...
- (void)sendAttachment:(AVObject *)object width:(double)dWidth height:(double)dHeight {
    [[CDSessionManager sharedInstance] sendAttachment:object toPeerId:self.mUser.username width:dWidth height:dHeight];
