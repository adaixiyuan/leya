...
@interface MeViewController () <CustomActionSheetDelegate> {
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ProfileInfoTableViewCell *profileInfoCell = (ProfileInfoTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"ProfileInfoCellID"];
...
- (void)onButThird:(UIView *)view {
    ...
    int originalCount = [[self.mUser objectForKey:@"report_count"] intValue];
    originalCount++;
    
    [AVCloud callFunctionInBackground:@"reportUser" withParameters:@{@"userId":self.mUser.objectId, @"reportCount":[NSNumber numberWithInt:originalCount]} block:^(id object, NSError *error)
     {
         if (!error) {
             [self showAlert:@"举报成功"];
         }
